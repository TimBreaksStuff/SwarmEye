...
- **Agent panes** — launch as many agents as you want, a terminal each, auto-arranged into a grid you can resize and rearrange (or place by hand with auto-organize off), with search, transcript export, dictation, one-click context clearing (`/clear`) and drag-and-drop file paths. Each header keeps restart, `/clear`, maximize, split and close in reach and folds export, search, dictation and the two text-size buttons behind a `⋯`; *Fixed agent pane buttons* in ⌨ Options puts all of them back inline. Text selection, clickable URLs and Ctrl+C copy work in every pane, even while the agent inside captures the mouse. The mouse wheel scrolls back through an agent's output where you point it — over the transcript it walks the history, over the input box at the bottom it still steps through the prompts you typed before; typing anything drops back to the live view.
...
- **Command palette** — `Ctrl+K` jumps to any agent, workspace, open task, installed skill or view, plus the verbs (new agent in a named workspace, message agents, search everything, options). Fuzzy: `tb` finds Task Board. Jumping to an agent switches workspace and view on the way.
