...
const Scope = {
  ...
  dismiss: (e) => {
    if (!Scope.menuEl.contains(e.target)) Scope.close();
  },
  ...
  async open(anchor, workspaceId, onPick) {
    ...
    // a pane's scope chip can sit low in the grid — flip the menu above the
    // anchor when it would run off the bottom, the branch menu's own move
