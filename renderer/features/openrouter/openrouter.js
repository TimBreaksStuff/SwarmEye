/* renderer/features/openrouter/openrouter.js — the OpenRouter key row in ⌨ Options → Setup.
...
export const OpenRouterUI = {
   ...
   * model; they differ in what wraps it — see agent/README.md. The choice
  ...
  _provEl: null,
  _provUndismiss: null, // dismissPop's teardown, live while the menu is up
  closeProviderMenu() {
    if (!this._provEl) return;
    this._provEl.remove();
    this._provEl = null;
    this._provUndismiss();
    this._provUndismiss = null;
  },
  openProviderMenu(anchorEl, onPick) {
    ...
    placePop(menu, anchorEl);
    menu.querySelector('.branch-item').focus(); // Enter = Claude, the pre-feature behaviour
    this._provEl = menu;
    this._provUndismiss = dismissPop(menu, () => this.closeProviderMenu(), { esc: true });
  },
  ...
  _menuEl: null,
  _menuUndismiss: null, // dismissPop's teardown, live while the menu is up
  closeModelMenu() {
    if (!this._menuEl) return;
    this._menuEl.remove();
    this._menuEl = null;
    this._menuUndismiss();
    this._menuUndismiss = null;
  },
  openModelMenu(anchorEl, onPick, { extra = [] } = {}) {
    ...
    document.body.appendChild(menu);
    /* The + Agent button hugs the right window edge and the slugs are long, so
     * the clamp needs the menu's real width — hence placing it after appending.
     ...
     * it but the harness row hung off the screen: `flip` moves it above the
     * anchor the way the branch and scope menus do, and `cap` — which this
     * menu needs and those don't — limits it to the room on whichever side it
     * lands, so a chip too low for either half still shows a scrollable list. */
    placePop(menu, anchorEl, { flip: true, cap: true });
    input.focus();
    this._menuEl = menu;
    // Escape goes through dismissPop rather than the input's own handler: that
    // one only fires while the input has focus, and Escape after a list click
    // fell through to whatever ESCAPABLE surface sat underneath
    this._menuUndismiss = dismissPop(menu, () => this.closeModelMenu(), { esc: true });
  },
};
