...
- `ptystream.js` — pty output on its way to the renderer. node-pty burst small chunks and each one forwarded is a renderer wake, so output coalesce per session: ~16ms for a pane on screen, 250ms for one nobody is looking at (same bytes, a fifteenth of the wake-ups). The renderer report which sessions are visible over `sessions:visible`. Was inline in `main.js`, which is a chokepoint file; it is a queue, two timers and one rule, so it is its own module.
...
No build step, **all ES modules**. `index.html` carry six `<script>` tags: five xterm UMD bundles from `node_modules` (they define `window.Terminal` and friends, which `pane.js` read, so they must come first), then `<script type="module" src="boot.js">`. Nothing else. There used to be twenty more in a hand-sorted order that was load-bearing twice over; every edge is an `import` now and each file say at its own top what it need.

Only six things live at `renderer/` top level: `boot.js`, `app.js`, `index.html`, `styles/`, `lib/`, `fonts/`. **Everything else is `features/<area>/`** — one folder per UI area, holding that area's JS, its CSS, **its markup** and its `README.md`.

**`index.html` is the shell only.** It held the DOM of every feature — the board, the skills screen, the coordinator, the notification panel, a 260-line Options popover — in the file two agents on unrelated features both had to edit. Each area's markup is now `features/<area>/<area>.html`, one or more `<template data-mount="name">`, and the shell keeps a `<div data-fragment="name">` placeholder per block. `lib/fragments.js` replace each placeholder with its template's content, so the resulting DOM is exactly what the inline markup gave. An area needing markup in two places in the shell (notifications: panel and bell popover) use two templates rather than moving one — moving would change which element paint on top where z-index tie. `fetch` over `file://` work in Electron's renderer, verified on 38; that why no build step needed.

**`boot.js` is the entry, not `app.js`.** Feature modules look their elements up and build their selects when evaluated, and a static `import` cannot wait for anything. boot.js `await mountFragments()`, then fill the model/effort tables from `models:list`, then `await import('./app.js')` — a dynamic import is what let the whole feature graph evaluate against a finished DOM and a filled vocabulary. **`main/models.js` own the model and effort tables**, same arrangement as `main/roles.js`: the renderer had a second copy with the labels in it, so a new tier was two edits in two processes. A fragment that fail to mount throw rather than leaving a feature wired to nothing; `boot-check.js` assert `window.__swarmTestState` exist, because a renderer that never booted still paint the shell and otherwise look fine.

**`app.js` is app state, the agent lifecycle, and IPC wiring — and that is all it should be.** What came out of it: `features/workspaces/` (add/remove/rename/reorder/select), `features/shortcuts/` (the keyboard map, the Escape chain's driver, the macOS label pass), and earlier `lib/toast.js` and `lib/keys.js`. What deliberately stays, because moving it would make it worse rather than better:

- **the agent lifecycle** (`mountPane`, `addAgent`, `spawnAgents`, `cloneActiveAgent`, `paneHandlers`, the session IPC handlers). It *is* the state and its transitions; a module for it would take a `ctx` of fifteen and own nothing the state does not.
- **the `ESCAPABLE` list.** Almost every entry pair another area's element with that area's own close function, and `app.js` already import all of them. `features/shortcuts/` drive the list; building it there would mean importing every area into the keyboard map.
- **the palette's entries.** `features/palette/` own the box and the filtering; the entries are built here because this is the one file that know both the state and what an entry should *do*, and they are rebuilt on every open so a closed agent is never still offered.

`lib/` the shared helpers no single area own — `dom.js`, `tooltip.js`, `confirm.js`, `icons.js`, `resizable.js`, `toast.js`, `keys.js`, and the two sheets `tooltip.css` and `toast.css`. `toast.js` and `keys.js` (`modHeld`) were in `app.js` published on `window`, purely because classic scripts could not import; that reason is gone.

**Cycles are the one real hazard.** `import` is fine in a cycle as long as nothing read across it *at module-evaluation time* — a `const X = (() => {…})()` body run then, so what it touch at top level matter. `board.js` built its mode/model selects from `Pane.MODES` at load; `Pane` import `openrouter.js` which import `board.js`, so `Pane` sat in the temporal dead zone and the renderer died on boot. Fixed by importing the tables from `pane-const.js` (a leaf, import nothing) instead of off the class — which the board, launch card, coordinator, orchestrator, OpenRouter and Options panel now all do. **Read a vocabulary from the file that declare it, not off a class's statics.** `node scripts/boot-check.js` catch this class of failure in one run.

Area stylesheet slot depend on which half it took — see **Styling** below; sheet carrying only `app.css`'s half go *before* `chrome-clean.css`, sheet that took chrome-clean's half too go after. Move only selectors exclusively that area's: shared class it happen to use (`.kbd-title`, also styled by notif-pop and board head) stay in sheet it came from. Watch for rule *later* in `chrome-clean.css` that override one you moving — move flip cascade. `settings.css` hit exactly that with flat-popover treatment, resolved by carrying effective values itself and coming off shared list. **`scripts/style-snapshot.js --cascade` prove a move rather than argue it.**

`features/pane/` is seven files, and **`index.js` is its front door**:

- `pane-theme.js` — xterm palettes, light-theme contrast pass, font size/weight state, and the exported setters that move it.
- `pane-const.js` — ⌨ Options mirrors and their setters, mode/model/effort tables the rest of the app import, tool sets, pure formatters.
- `pane.js` — the class: constructor that build header DOM, buffer scans, terminal lifecycle, statics, the glyph-atlas counters.
- `pane-status.js`, `pane-usage.js`, `pane-git.js` — 31 methods re-attached with `Object.assign(Pane.prototype, {…})`. They import `pane.js` direct, never `index.js`, which would be a cycle.
- `index.js` — import the class, then the three mixins, then re-export. **Import `Pane` from here**: the one in `pane.js` have no `syncStatus`, no usage panel, no git chip. A module's imports evaluate before its body, so this enforce what the `<script>` order used to only ask for.

A `let` one file declare and another assign to worked while these shared a script scope. It is illegal between modules, so the five Options flags and the theme/font state got exported setters in the file that own them, and the atlas counters moved into `pane.js`, the only file that ever touched them. **Class body separate methods by nothing, object literal need commas** — the one real hazard when moving method group; check count of `^  }$` terminators equal count of method headers before and after.

Three full-screen views swap into one slot: agent grid, Task Board (`features/board/board.js`), Skills — each toggle explicitly hide the other two, so a fourth mean touch all of them. **Prefer popover over fourth view**: `features/palette/`, `features/coordinator/` all float over whatever view up, build own root (so `index.html` gain only script tag), cost none of that wiring. New popover do need line in `ESCAPABLE` in `app.js` — ordered list, outermost first, first open one win — and get drag-to-resize from `lib/resizable.js` (CSS `resize: both` do resizing; module only remember size and centre box). Anchor it with `placePop` from `lib/dom.js` and close it with `dismissPop`, never hand-rolled `getBoundingClientRect` maths plus a document listener: ten popovers had own copy before the sweep. `placePop` measure the box, so unhide it *first*; `dismissPop` hand back its teardown, which your own `close()` must call.
