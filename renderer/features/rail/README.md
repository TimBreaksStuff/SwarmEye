...
One area, three files: the workspace tiles, their right-click menu and the
gauges (`topbar.js`), the fold-out agent rows under a tile (`wsagents.js`), and
the drag handles that size and collapse it (`railgrip.js`).
...
the Add agents chips / Rename / Remove, drag the grip to resize and collapse,
and watch the gauges while agents run. A chip starts that many agents in the
workspace under the cursor — it becomes the selected one — and chips past the
free slots under the agent cap are greyed out.
