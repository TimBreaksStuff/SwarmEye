...
- **Swarm map** — a compact status grid in the left rail, above the `Task Board` tile: one slot per agent showing busy, needs attention, idle, or free at a glance across every workspace, each slot bordered in its workspace's colour. Click any lit slot to jump to that agent and focus its pane; hover for its name.
